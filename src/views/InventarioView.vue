      ...
      <div v-else class="tabla-container">
        <table class="tabla-generica">
          ...
          <tbody>
            ...
            >
              <td>
                <code class="codigo-badge">{{ insumo.codigo_unico || 'S/C' }}</code>
              </td>
              <td>{{ insumo.nombre }}</td>
              <td>{{ insumo.stock }}</td>
      ...
      <div v-else class="tabla-container">
        <table class="tabla-generica">
          ...
          <tbody>
            ...
            <tr v-else v-for="proveedor in proveedores" :key="proveedor.ruc">
              <td>{{ proveedor.ruc }}</td>
      ...
      <div class="modal-box">
        ...
        <form @submit.prevent="guardarInsumo">
          <div class="campo">
            ...
            <input 
              v-model="nuevoInsumo.codigo_unico" 
              type="text" 
          ...
          <p v-if="errorFormulario" class="estado-error-texto">{{ errorFormulario }}</p>
...
const guardarInsumo = async () => {
  nuevoInsumo.value.codigo_unico = nuevoInsumo.value.codigo_unico.trim().toUpperCase()
  nuevoInsumo.value.nombre = nuevoInsumo.value.nombre.trim()

  if (!nuevoInsumo.value.codigo_unico) {
    errorFormulario.value = 'El código único es obligatorio.'
    return
  }

  if (!nuevoInsumo.value.nombre) {
    errorFormulario.value = 'El nombre del insumo es obligatorio.'
    return
  }
...
const eliminarInsumo = async (id: string) => {
  const confirmar = window.confirm('¿Seguro que quieres eliminar este insumo?')
